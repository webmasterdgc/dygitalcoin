...
#if defined(NDEBUG)
# error "Dygitalcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe3209b561ed59fb0285c2116f0462b0b2818ad7d8807ef6626159a30e0cec68f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dygitalcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dygitalcoin Signed Message:\n";
...
{
    // Dygitalcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 190000); // Dygitalcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Dygitalcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Dygitalcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dygitalcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dygitalcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4e88a4567f4efded7ebd55082fd89bb7df153127645b4bf02a20921a5a902c54");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "I am slow Walker but Never Walk Backwards 2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04812d5d42f114de7332d42513265174a6f01a59e5c0a2097d23a023db469a66032646539d8eb51cb90187a924dc5e10cd682c8846e503466cc3a191a6ad85d3b7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1527095339;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085414289;

        if (fTestNet)
        {
            block.nTime    = 1527095363;
            block.nNonce   = 388757645;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x97ae67b707fe47b3f9de0aa81606022c176adc9bd6c710d8f3d75815297b0060"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd2 }; // Dygitalcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DygitalcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DygitalcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DygitalcoinMiner(CWallet *pwallet)
{
    printf("DygitalcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dygitalcoin-miner");
    ...
    try { loop {
        ...
        printf("Running DygitalcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DygitalcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DygitalcoinMiner, pwallet));
}
